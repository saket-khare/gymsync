  ...
  return (
    <div className="px-6 py-5">
      <div className="grid grid-cols-2 md:grid-cols-4 gap-x-8 gap-y-8 text-sm">
        <div className="space-y-3">
          ...
          <div>
            <p className="text-zinc-200 font-medium">
              {member.age} yrs <span className="text-zinc-600 font-normal mx-1">·</span>{' '}
              <span className="capitalize">{member.gender}</span>
            </p>
            <p className="text-zinc-400 text-xs mt-1">
              {member.weightKg}kg <span className="text-zinc-700 mx-1">·</span> {member.heightCm}
              cm
            </p>
          </div>
        </div>
        ...
        <div className="space-y-3">
          <p className="text-xs font-medium text-zinc-500 uppercase tracking-wider">Experience</p>
          <div>
            <p className="text-zinc-200 font-medium capitalize">
              {member.gymExperience.replace('_', ' ')}
            </p>
            <div className="flex items-center gap-1.5 mt-1">
              <p className="text-zinc-400 text-xs">Self-rated:</p>
              <div className="flex gap-0.5">
                {[1, 2, 3, 4, 5].map((star) => (
                  <div
                    key={star}
                    className={`w-1.5 h-1.5 rounded-full ${
                      star <= member.selfRatedFitness ? 'bg-indigo-400' : 'bg-zinc-800'
                    }`}
                  />
                ))}
              </div>
        ...
        <div className="space-y-3">
          ...
          <div>
            <p className="text-zinc-200 font-medium">{member.daysPerWeekAvailable}x / week</p>
            <p className="text-zinc-400 text-xs mt-1">
        ...
        <div className="space-y-3">
          <p className="text-xs font-medium text-zinc-500 uppercase tracking-wider">Lifestyle</p>
          <div>
            <p className="text-zinc-200 font-medium capitalize">
        ...
        <div className="space-y-3">
          <p className="text-xs font-medium text-zinc-500 uppercase tracking-wider">Timeline</p>
          <div>
            <p className="text-zinc-200 font-medium">{member.timelineMonths} months</p>
            <p className="text-zinc-400 text-xs mt-1 capitalize">{member.goalUrgency} urgency</p>
          </div>
        </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {member.injuries && (
                <div className="bg-rose-500/5 border border-rose-500/10 rounded-lg p-3">
                  ...
                  <span className="text-sm text-zinc-200 leading-relaxed">{member.injuries}</span>
                </div>
              )}
